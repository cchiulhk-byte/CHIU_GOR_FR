...
const getCancelledEmailHtml = (studentName: string, courseType: string, date: string, time: string, method: string | null) => `
      ...
      <div style="background: #fef2f2; border: 1px solid #fecaca; border-radius: 10px; padding: 20px 24px; margin-bottom: 28px;">
        <table style="width: 100%; border-collapse: collapse; font-size: 14px;">
          <tr>
            ...
            <td style="color: #111827; font-weight: 600; padding: 6px 0;">${courseType}</td>
          </tr>
          <tr>
            <td style="color: #9ca3af; padding: 6px 0;">Date</td>
            <td style="color: #111827; font-weight: 600; padding: 6px 0;">${date}</td>
          </tr>
          <tr>
            <td style="color: #9ca3af; padding: 6px 0;">Time</td>
            <td style="color: #111827; font-weight: 600; padding: 6px 0;">${time} HKT</td>
          </tr>
          <tr>
            <td style="color: #9ca3af; padding: 6px 0;">Payment Method</td>
            <td style="color: #111827; font-weight: 600; padding: 6px 0; text-transform: capitalize;">${method || "—"}</td>
          </tr>
        </table>
      </div>
...
const getConfirmedEmailHtml = (studentName: string, courseType: string, date: string, time: string, method: string | null) => `
  <div style="font-family: 'Helvetica Neue', Arial, sans-serif; max-width: 560px; margin: 0 auto; background: #fff; border-radius: 12px; overflow: hidden; border: 1px solid #f0f0f0;">
  ...
  try {
    ...
    if (payload.action === "resend_email") {
      ...
      if (booking.status === "confirmed") {
        ...
        emailHtml = getCancelledEmailHtml(
          booking.student_name,
          booking.course_type,
          booking.preferred_date,
          booking.preferred_time,
        ...
        return new Response(
          JSON.stringify({
            success: false,
            error: `Cannot resend email for booking with status: ${booking.status}`,
          }),
          {
            status: 400,
            headers: { ...corsHeaders, "Content-Type": "application/json" },
